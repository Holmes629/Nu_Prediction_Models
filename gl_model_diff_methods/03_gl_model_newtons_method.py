...
def solve(Pr, Ra):
    ...
    eq1 = (Nu - 1)*Ra*(Pr**(-2)) - comp_1  # Corrected equation
    eq2 = Nu - comp_2 - comp_3

    # Jacobian matrix
    Jacobian = [[diff(eq1, Nu), diff(eq1, Re)],
                [diff(eq2, Nu), diff(eq2, Re)]]
    ...
    for i in range(max_iter):
        # Substitute current values into the equations
        current_eq_values = [eq1.subs({Nu: initial_guess[0], Re: initial_guess[1]}),
                            eq2.subs({Nu: initial_guess[0], Re: initial_guess[1]})]

        try:
            # Calculate the increment using the inverse Jacobian
            increment = solve([eq1, eq2], [Nu, Re])
            increment_Nu = float(increment[Nu])
            increment_Re = float(increment[Re])
        except Exception as e:
            print("Error in solving equations:", e)
            break

        # Update the guess
        initial_guess[0] -= increment_Nu
        initial_guess[1] -= increment_Re

        # Check for convergence
        if max(abs(current_eq_values)) < tolerance:
            print("Converged!")
            break
